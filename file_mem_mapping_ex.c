...
int do_read()
{
    ...
    while(n < 5)
    {
        if (memcmp(info, &local, sizeof(struct shared_data))) { // compare memory area
            printf("update => pid %d, counter %d\n", info->pid, info->counter);
            memcpy(&local, info, sizeof(struct shared_data)); // copy memory area
